              ...
              declareVar(_valueVar)
                  .assign(
                    refer('$_resultVar.data').conditionalIsNullIf(
                      thisNullable: returnType.isNullable,
                      whenFalse: refer('await workerManager.execute').call([
                        refer(
                          '() => deserialize${_displayString(innerReturnType)}List($_resultVar.data!.cast<Map<String,dynamic>>())',
                        ),
                      ]),
                    ),
                  )
...
''');
                ...
                (e) async => MapEntry(
                    e.key,
                    await workerManager.execute(() => deserialize${_displayString(type)}List((e.value as List).cast<Map<String, dynamic>>())))
            ''');
                break;
            }
            if (future) {
...
''');
                ...
                mapperCode = refer('''
                (e) async => MapEntry(
                    e.key, await workerManager.execute(() => deserialize${_displayString(secondType)}(e.value as Map<String, dynamic>)))
            ''');
                break;
            }
            if (future) {
        ...
        if (_isBasicType(returnType)) {
          ...
          switch (clientAnnotation.parser) {
            ...
            case retrofit.Parser.FlutterCompute:
              mapperCode = refer(
                'await workerManager.execute(() => deserialize${_displayString(returnType)}($_resultVar.data!))',
              );
              break;
          }
          blocks.add(
    ...
    final queryParameters = queries.map((p, r) {
      ...
      if (_isBasicType(p.type) ||
        ...
        switch (clientAnnotation.parser) {
          ...
          case retrofit.Parser.FlutterCompute:
            value = refer(
              'await workerManager.execute(() => serialize${_displayString(p.type)}(${p.displayName}))',
            );
            break;
        }
      }
          ...
          case retrofit.Parser.FlutterCompute:
            blocks.add(
              declareFinal(dataVar)
                  .assign(
                    refer('''
            await workerManager.execute(() => serialize${_displayString(_genericOf(bodyName.type))}List(${bodyName.displayName}))
            '''),
                  )
                  .statement,
            );
              ...
              case retrofit.Parser.FlutterCompute:
                if (bodyName.type.nullabilitySuffix !=
                  ...
                  blocks.add(
                    refer('$dataVar.addAll').call([
                      refer(
                        'await workerManager.execute(() => serialize${_displayString(bodyName.type)}(${bodyName.displayName}))',
                      )
                    ]).statement,
                  );
                } else {
  ...
  bool _missingSerialize(CompilationUnitElement ele, DartType type) {
    switch (clientAnnotation.parser) {
      ...
      case retrofit.Parser.MapSerializable:
        return false;
      case retrofit.Parser.FlutterCompute:
        log.warning('clientAnnotation.parser : ${clientAnnotation.parser} :: ele.functions ${ele.functions.length}');
        return !ele.functions.any(
          (element) {
            log.warning(
                'element name :: ${element.name} :: params length: ${element.parameters.length} :: displayString:: ${_displayString(element.parameters[0].type)} == ${_displayString(type)}');
            return element.name == 'serialize${_displayString(type)}' &&
